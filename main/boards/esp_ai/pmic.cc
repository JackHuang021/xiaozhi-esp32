 ...
 * @param state 
 */
void Pmic::set_lcd_cs(bool state) {
 ...
 * @param brightness 
 */
void Pmic::set_lcd_backlight(uint8_t brightness) {
    uint8_t value = ReadReg(0x90);
    uint16_t voltage = 0;
    /* daxian 2.4 inch lcd, backlight voltage 2.7V ~ 3.4V，max current is 80mA */
    const uint16_t min_brightness_vol = 2700;
    ...
    voltage = 2700 + (brightness / 10) * 100;
 ...
 * @param enable 
 */
void Pmic::set_pa_enable(bool enable)
{
    uint8_t value = ReadReg(0x90);
